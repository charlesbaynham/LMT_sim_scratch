...
def render_filmstrip(
    ...
    *,
    ...
    x_edges, z_edges = pixel_grid(
        non_empty, n_x=n_x, n_z=n_z, x_pad_min=x_pad_min, z_pad_min=z_pad_min
    )
...
def centroid_um(branches):
    if not len(branches):
        return None, None
    w = branches[:, 3]
    return (np.sum(branches[:, 0] * w) / w.sum()) * 1e6, (
        np.sum(branches[:, 1] * w) / w.sum()
    ) * 1e6


print(
    f"{'Label':10s}  {'x_cen µm':>10s}  {'z_cen µm':>10s}  {'Δx err µm':>10s}  {'Δz err µm':>10s}"
)
for i, (label, sg) in enumerate(zip(LABELS, snap_g2)):
    ...
    if xc is not None:
        ...
        print(
            f"{label:10s}  {xc:+10.2f}  {zc:+10.2f}  {xc - ex:+10.4f}  {zc - ez:+10.4f}"
        )
...
print(
    f"Expected σ_z at t=10 ms: {sig_final * 1e6:.0f} µm  "
...
print(
    f"Pixel size: {pixel_size * 1e6:.0f} µm  →  {n_x}×{n_z} grid "
...
fig3 = render_filmstrip(
    ...
    n_x=n_x,
    n_z=n_z,
    x_pad_min=0.5 * pixel_size,
    z_pad_min=0.5 * pixel_size,
    common_scale=False,
)
plt.show()
