...
def render(branches, x_edges, z_edges, *, blur_sigma_m=SINGLE_ATOM_WAVEPACKET_SIGMA_M):
    ...
    if blur_sigma_m:
        sigma_pix = (
            blur_sigma_m / (z_edges[1] - z_edges[0]),
    ...
    if branches.shape[1] >= 7:
        ...
        atoms = (branches,)
    for atom in atoms:
        re, _, _ = np.histogram2d(
            atom[:, 1], atom[:, 0], bins=(z_edges, x_edges), weights=atom[:, 4]
        )
        im, _, _ = np.histogram2d(
            atom[:, 1], atom[:, 0], bins=(z_edges, x_edges), weights=atom[:, 5]
        )
...
def plot_camera_shot(
    ...
    *,
    ...
    n_x=21,
    n_z=48,
    blur_sigma_m=SINGLE_ATOM_WAVEPACKET_SIGMA_M,
    x_pad_frac=0.25,
    z_pad_frac=0.15,
):
...
def plot_filmstrip(
    ...
    *,
    ...
    """
    snap_g, snap_e = collect_filmstrip(
        pulse_sequence,
        velocities,
        discard_threshold=discard_threshold,
        progress=progress,
    ...
    fig.suptitle(
        title
        or f"Filmstrip (final excited fraction = {excited_fraction:.2f}; "
