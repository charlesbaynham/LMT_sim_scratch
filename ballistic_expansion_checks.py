...
def render_filmstrip_figure(
    ...
    *,
    ...
    for col, (ax, img, label) in enumerate(zip(axes, imgs, labels)):
        ...
        ax.imshow(
            img, origin="lower", aspect="auto", extent=extent,
...
def check_2_drifting_atom():
    ...
    print(f"  Expected drift after 10 ms: Δz = {VZ*N_STEPS*DT*1e6:.0f} µm,  "
    ...
    for i, (label, sg) in enumerate(zip(LABELS, snap_g)):
        ...
        if xc is not None:
            ...
            print(f"    {label}: ({xc:+7.1f}, {zc:+7.1f}) µm  "
...
def check_3_thermal_cloud():
    ...
    print(f"  σ_pos = {SIGMA_POS*1e6:.0f} µm,  σ_v = {sigma_v*1e3:.2f} mm/s per axis")

    expected_sigma_final = np.sqrt(SIGMA_POS**2 + (sigma_v * N_STEPS * DT)**2)
    print(f"  Expected cloud σ after {N_STEPS} ms: "
    ...
    fig = render_filmstrip_figure(
        ...
        channel="ground",
        n_x=40, n_z=80,
        x_pad_min=50e-6, z_pad_min=50e-6,
